            ...
            <group ref={groupRef}>
                ...
                <mesh position={[wallOffsetX, 0, 0]} geometry={wallWithHoleGeometry}>
                    {isPaper ? (
                        ...
                        <meshStandardMaterial color="#111827" roughness={0.35} metalness={0.8} side={THREE.DoubleSide} />
                    )}
                </mesh>
                ...
                {/* Baseboard / LED Guides left/right/top */}
                {isPaper ? (
                    <>
                    ...
                    <>
                        {/* Baseboard - Replaced with Glowing Neon LED Strip */}
                        <mesh position={[wallOffsetX - 1.4, -CORRIDOR_HEIGHT / 2 + 0.05, 0.02]}>
                            <planeGeometry args={[doorBoardWidth, 0.04]} />
                            <meshBasicMaterial 
                                color={side === 'left' ? "#00F5FF" : "#FF2BD6"}
                                toneMapped={false}
                                side={THREE.DoubleSide}
                        ...
                        {/* Top Trim - Replaced with Glowing Neon LED Strip */}
                        <mesh position={[wallOffsetX - 1.4, CORRIDOR_HEIGHT / 2 - 0.05, 0.02]}>
                            <planeGeometry args={[doorBoardWidth, 0.03]} />
                            <meshBasicMaterial 
                                color={side === 'left' ? "#FF2BD6" : "#00F5FF"}
                                toneMapped={false}
                                side={THREE.DoubleSide}
                        ...
                        {/* Top Trim (Listwa) Right side of door */}
                        <mesh position={[wallOffsetX + 1.4, CORRIDOR_HEIGHT / 2 - 0.05, 0.02]}>
                            <planeGeometry args={[doorBoardWidth, 0.03]} />
                            <meshBasicMaterial 
                                color={side === 'left' ? "#FF2BD6" : "#00F5FF"}
                ...
                {(() => {
                    ...
                    if (isPaper) {
                        ...
                        threshTex.repeat.set(THRESH_W / 1.5, 1);
                        return (
                            <mesh
                ...
                <group position={[wallOffsetX, -0.4, 0]}>
                    ...
                    <group position={[0, doorHeight / 2 + 0.45, 0.08]}>
                        ...
                        {label === 'THE GALLERY' && (
                            <group position={[0, 0, 0.01]}>
                                <Text
                                    font={isPaper ? "/fonts/CabinSketch-Bold.ttf" : CYBER_FONT_URL}
                                    fontSize={isPaper ? 0.25 : 0.16}
                                    color={isPaper ? "#111111" : (side === 'left' ? "#00F5FF" : "#FF2BD6")}
                                    anchorX="center"
                                    anchorY="bottom"
                                    position={[0, -0.01, 0]}
                                    toneMapped={!isPaper}
                    ...
                    <group ref={doorRef} position={[doorPivotX, 0, 0.01]}>
                        ...
                        {isPaper ? (
                            ...
                            <>
                                {/* Cyber Glass Door Panel */}
                                <mesh position={[doorMeshX, -0.2, -0.002]}>
                                    <planeGeometry args={[doorWidth, doorHeight]} />
                                    <meshStandardMaterial 
                                        color="#161b2f"
                                        transparent
                                ...
                                {/* Neon Outline Border */}
                                <mesh position={[doorMeshX, -0.2, -0.001]}>
                                    <planeGeometry args={[doorWidth, doorHeight]} />
                                    <meshBasicMaterial 
                                        color={side === 'left' ? "#00F5FF" : "#FF2BD6"}
                                        wireframe
                                ...
                                {/* Door Back Panel (Glass/Reflective) */}
                                <mesh
                                    position={[doorMeshX, -0.2, -0.015]}
                                    rotation={[0, Math.PI, 0]}
                                    scale={[side === 'right' ? -1 : 1, 1, 1]}
                                >
                                    <planeGeometry args={[doorWidth, doorHeight]} />
                                ...
                                {/* Ref placeholders for GSAP compatibility */}
                                <mesh ref={doorPaintedRef} position={[0,0,0]} visible={false} />
                                <mesh position={[0,0,0]} visible={false}>
                                    <revealMaterial ref={doorMaterialRef} map={doorTexture} uProgress={0} />
                                </mesh>
                            </>
                        )}

                        {/* Handles (Sketched vs Chrome) */}
                        <group ref={handleRef} position={[doorMeshX + (side === 'left' ? 0.45 : -0.45), -0.29, 0.03]}>
                            {isPaper ? (
                                <>
                                    {/* Painted handle (behind) */}
                                ...
                                <>
                                    ...
                                    {/* Ref placeholders for GSAP compatibility */}
                                    <mesh ref={handlePaintedRef} position={[0,0,0]} visible={false} />
                                    <mesh position={[0,0,0]} visible={false}>
                                        <revealMaterial ref={handleMaterialRef} map={handleTexture} uProgress={0} />
                                    </mesh>
                                </>
                            )}
                        </group>
