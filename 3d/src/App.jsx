...
import ThemeCanvasBridge from './theme/ThemeCanvasBridge';
...
// Initialize PostHog only when a valid token exists
...
import Return2DButton from './components/ui/Return2DButton';
...
function AppContent() {
  ...
  const { isTransitioning, transitionType, signalSceneReady, hasTransitionedFrom2D } = useDimension();
  const isFrom2D = hasTransitionedFrom2D || (isTransitioning && transitionType === 'enter');

  // Toggle body mode-3d class on mount/unmount
  useEffect(() => {
  ...
  // When 3D scene is ready, mark loaded and signal transition completion
  useEffect(() => {
    if (sceneReady) {
      setIsLoaded(true);
      signalSceneReady();
