      ...
      <div className="dashboard-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
        ...
        <div className="glass-card featured-product" style={{ gridColumn: '1 / -1', background: 'linear-gradient(135deg, color-mix(in srgb, var(--color-accent-primary) 15%, transparent), color-mix(in srgb, var(--color-accent-secondary) 15%, transparent))' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '2rem' }}>
            <div>
              <h2 style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>SoulWake Core</h2>
              ...
              <a href="https://soulwake.app" target="_blank" rel="noreferrer" className="hero__cta-button primary" style={{ textDecoration: 'none', background: 'var(--color-accent-primary)', color: 'var(--color-background)', padding: '0.8rem 1.5rem', borderRadius: '6px', fontWeight: 'bold', display: 'inline-block' }}>
                Launch SoulWake <i className="fa-solid fa-rocket" style={{ marginLeft: '10px' }}></i>
              </a>
            </div>
        ...
        <div className="glass-card">
          <h3 style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
            Active Focus Goals
          ...
          {activeTasks.length === 0 ? (
            ...
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {activeTasks.map(task => (
                <li key={task.id} style={{ display: 'flex', gap: '1rem', color: 'var(--color-text-secondary)', alignItems: 'center' }}>
                  <input 
                    type="checkbox" 
                  ...
                  />
                  <span style={{ fontSize: '1.05rem', color: 'var(--color-text-primary)' }}>{task.title}</span>
                </li>
              ))}
            </ul>
          )}
