...
const GamePage = () => {
  ...
  useEffect(() => {
    ...
    // --- Game Classes (Copied from your script.js) ---
    ...
    class Game {
      ...
      restart() {
        // Re-create the game instance to reset all state
        gameInstance = new Game();
        gameInstance.gameLoop(0); // Restart the loop
      }
    ...
    // --- Start the game ---
    gameInstance = new Game();
    gameInstance.gameLoop(0);

    // Cleanup function to remove event listener when the component unmounts
    return () => {
      document.removeEventListener('keydown', handleKeyDown);
      // We can also set gameOver to true to stop the loop, although it's not strictly necessary
      if (gameInstance) {
  ...
  return (
    <div className="game-container">
      <h1>Canvas Jumper Game</h1>
      <canvas ref={canvasRef} id="gameCanvas" />
      <p>Press SPACE to jump and restart!</p>
    </div>
  );
};
